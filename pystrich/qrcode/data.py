...
class QRCodeEncodingRule(NamedTuple):
    """Concrete :class:`~pystrich.charset.EncodingRule` for QR Code."""

    charset: Charset
    max_codepoint: int


_ENCODING_RULES: dict[QRCodeEncoding, QRCodeEncodingRule] = {
    "ascii": QRCodeEncodingRule("ascii", 0x7F),
    "iso-8859-1": QRCodeEncodingRule("iso-8859-1", 0xFF),
    "utf-8": QRCodeEncodingRule("utf-8", 0x10FFFF),
}

_AUTO_ENCODING_RULES: tuple[QRCodeEncodingRule, ...] = tuple(_ENCODING_RULES.values())
...
class QRCodeData:
    ...
    def __init__(
        ...
        *segments: str,
        ...
        if encoding is not None and encoding not in _ENCODING_RULES:
            raise PyStrichInvalidOption(
                f"unknown QRCodeData encoding {encoding!r}; "
        ...
        if auto_encoding:
            ...
            if max_codepoint > rule.max_codepoint:
                ...
                raise PyStrichInvalidInput(
                    f"QRCodeData encoding {encoding!r} expects {rule.charset.upper()}; "
    ...
    def __add__(self, other):
        if isinstance(other, str):
            ...
            other_auto = False
        elif isinstance(other, QRCodeData):
            if not (self.auto_encoding or other.auto_encoding) and other.encoding != self.encoding:
                raise PyStrichInvalidOption(
                    f"cannot concatenate QRCodeData with different encodings "
