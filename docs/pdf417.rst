...
.. title:: Generating a PDF417 barcode with Python

PDF417
======

pyStrich is a Python library for generating PDF417 barcodes, along with
other 1D and 2D symbologies.

PDF417 is a stacked 2D barcode used on driver's licences, boarding
passes, shipping labels and other documents that need high capacity
(up to 925 data codewords).

.. versionadded:: 0.14
   PDF417 support was added in this release.
...
Here's how to generate a PDF417 barcode and save it as an SVG:
...
are added on top). pyStrich picks both automatically to minimise the
symbol's area while keeping the wide aspect scanners expect at the
default ``row_height=3``.
...
:class:`~pystrich.exceptions.PyStrichInvalidPayloadLength` if the data does not
...
``"ascii"``       Raises :class:`~pystrich.exceptions.PyStrichInvalidInput` on any
                  byte > 127.
``"iso-8859-1"``  Latin-1 -- the PDF417 default character interpretation. Emits
                  codeword 927 + 3 (ECI 000003) once at the start of the symbol
                  so decoders that guess the charset don't misread short
                  high-byte payloads.
...
.. doctest::
   ...
   Traceback (most recent call last):
       ...
   pystrich.exceptions.PyStrichInvalidInput: PDF417Data encoding ASCII cannot encode the input; try PDF417Data('Ich dachte, Sie wären kräftiger', encoding='iso-8859-1') or pass auto_encoding=True to select an encoding automatically.
